...
resource "aws_vpc" "vpc" {
  ...
  tags = merge({ "Name" : var.vpc_name }, var.custom_tags, local.base_tags)
...
resource "aws_internet_gateway" "igw" {
  ...
  vpc_id = aws_vpc.vpc.id
  tags   = merge({ "Name" : format("%s-IGW", var.vpc_name) }, var.custom_tags, local.base_tags)
}
...
resource "aws_route_table" "public_route_table" {
  ...
  vpc_id = aws_vpc.vpc.id
  tags   = merge(local.base_tags, { "Name" : format("%s-PublicRouteTable", var.vpc_name) }, var.custom_tags)
}

resource "aws_route_table" "private_route_table" {
  count  = var.az_count
  vpc_id = aws_vpc.vpc.id
  tags   = merge(local.base_tags, { "Name" : format("%s-PrivateRouteTable%d", var.vpc_name, count.index + 1) }, var.custom_tags)
}
